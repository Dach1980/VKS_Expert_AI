...
(function installNormMetadataFix() {
  ...
  function canonicalTitle(number) {
    var normalized = String(number || '').replace(/\s+/g, ' ').trim().toLowerCase();
  ...
  function normalizeState() {
    ...
    data.forEach(function (norm) {
      ...
      versions.forEach(function (version) {
        ...
        if (change !== null && String(version.change_number || '') !== change) { version.change_number = change; changed = true; }
  ...
  console.log('[VKS Expert AI][Norms] metadata compatibility fix v6 loaded');
})();
