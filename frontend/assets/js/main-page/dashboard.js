...
function renderDashboardTable() {
  ...
  if (!tbody) return;
  var recentChecks = checksData.filter(function (c) { return c && (c.type === 'violation' || c.type === 'compliant' || c.type === 'unchecked'); }).slice(0, 5);
  ...
  for (var i = 0; i < recentChecks.length; i++) {
    ...
    var sevColor = c.type === 'violation' ? (c.severity === 'critical' ? 'danger' : c.severity === 'major' ? 'warning' : 'info') : c.type === 'compliant' ? 'success' : 'warning';
    var severityLabel = c.type === 'violation' ? (c.severity === 'critical' ? 'Критическое' : c.severity === 'major' ? 'Значительное' : 'Незначительное') : c.type === 'compliant' ? 'Соответствие' : 'Не проверено';
    html += '<tr><td>' + escapeHtml((c.docName || c.title || '').substring(0, 50)) + '</td><td>' + escapeHtml(c.section || 'ВК') + '</td><td>' + escapeHtml(c.date || c.checked_at || '') + '</td><td><span class="status-badge ' + sevColor + '">' + severityLabel + '</span></td><td>' + (c.type === 'violation' ? '1' : '0') + '</td><td><button class="btn btn-secondary btn-sm" onclick="navigateTo(\'checks\')">Подробнее</button></td></tr>';
  }
...
function updateDashboardMetrics() {
  ...
  if (metricDuration) {
    ...
    metricDuration.textContent = durations.length ? (durations.reduce(function (a, b) { return a + b; }, 0) / durations.length / 60).toFixed(1) + ' мин' : '—';
  }
  document.querySelectorAll('.metric-change').forEach(function (el) { el.textContent = total ? 'По фактическим данным' : 'Нет данных'; el.className = 'metric-change'; });
...
function renderDashboard() { renderDashboardTable(); updateDashboardMetrics(); }
