...
// The uploaded filename is the only source of truth for the amendment number.
// Parsed PDF/JSON metadata must not relabel a version.
...
function renderVersionActions(norm, version) {
  var sid = version.document_id || norm.id;
  var vid = version.version_id || version.id;
  return renderIndexAction(norm, version)
...
function renderNorms() {
  ...
  getNormsData().forEach(function (norm) {
    ...
    html += '<div class="norm-card" data-norm-card="' + esc(norm.id) + '">'
      ...
      + '<div class="norm-card-subtitle">' + esc(norm.title || norm.number || '') + '</div>'
      + '</div></div>'
  ...
  grid.querySelectorAll('.norm-version-index').forEach(function (button) {
    button.onclick = function (event) {
      event.preventDefault();
      event.stopPropagation();
      indexNormVersion(button.dataset.card, button.dataset.source, button.dataset.version);
    };
  });
  grid.querySelectorAll('.norm-version-activate').forEach(function (button) {
    button.onclick = function (event) {
      event.preventDefault();
      event.stopPropagation();
      activateNormVersion(button.dataset.card, button.dataset.source, button.dataset.version);
    };
  });
  grid.querySelectorAll('.norm-version-delete').forEach(function (button) {
    button.onclick = function (event) {
      event.preventDefault();
      event.stopPropagation();
      deleteNormVersion(button.dataset.card, button.dataset.source, button.dataset.version);
    };
  });
}
...
function indexNorm(id) {
  ...
  if (!norm || !norm.version_id) return toast('Сначала назначьте действующую версию', 'error');
  ...
  normsPollTimers[key] = setInterval(async function () {
    try {
      ...
      if (version.processing.error) {
        ...
        toast('Индексация завершилась с ошибкой: ' + version.processing.error, 'error');
...
function deleteNorm(id) {
  ...
  if (norm && norm.versions && norm.versions.length) {
    deleteNormVersion(id, norm.id, norm.versions[0].version_id || norm.versions[0].id);
  }
}
...
function handleNormDropzoneClick() {
  ...
  input.onchange = function (event) { handleNormFiles(event.target.files); };
  input.click();
}
...
function handleNormDrop(event) {
  ...
  if (event.dataTransfer && event.dataTransfer.files.length) handleNormFiles(event.dataTransfer.files);
}
async function handleNormFiles(files) {
  ...
  for (var i = 0; i < queue.length; i++) {
    try { await uploadNormFile(queue[i]); ok++; }
    catch (error) { failed++; }
  }
  await loadNorms();
  toast('Загрузка завершена: ' + ok + ' из ' + queue.length + (failed ? ' (ошибок: ' + failed + ')' : ''), failed ? 'error' : 'success');
}
async function uploadNormFile(file) {
  if (!file.name.toLowerCase().endsWith('.pdf')) {
    toast('Файл «' + file.name + '» пропущен: поддерживается только PDF.', 'error');
    throw new Error('format');
  }
  var form = new FormData();
  form.append('file', file, file.name);
  var response = await fetch(NORMS_API_BASE + '/upload', { method: 'POST', body: form });
  var data = {};
  try { data = await response.json(); } catch (e) {}
  ...
  if (!response.ok) throw new Error(data.detail || ('HTTP ' + response.status));
  toast('Файл «' + file.name + '» загружен. Версия ожидает выбора действующей редакции.', 'success');
  return data;
}
