...
function searchIcon() { return '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="7"></circle><path d="m20 20-3.5-3.5"></path></svg>'; }
function renderVersionActions(n, v, sid, vid) {
  var state = indexing(v) ? '<span class="status-badge info">Происходит индексация</span>' : indexed(v) ? '<span class="status-badge success">Индексировано</span>' : '<button type="button" class="btn btn-primary btn-sm norm-version-index" data-card="' + esc(n.id) + '" data-source="' + esc(sid) + '" data-version="' + esc(vid) + '">' + searchIcon() + 'Индексировать</button>';
...
function renderNormVersions(n) {
  ...
  var h = '<div id="normVersions-' + esc(n.id) + '" class="norm-versions-panel" style="display:none"><div class="norm-versions-title">Загруженные версии</div>';
  vs.forEach(function (v) {
    ...
    h += '<div class="norm-version-row"><div class="norm-version-info-block"><strong>' + versionLabel(v) + '</strong><div class="norm-version-filename">' + esc(date) + ' · ' + esc(v.original_filename || v.filename || vid) + '</div><div class="norm-version-pages">📄 ' + pages + ' стр.</div></div><div class="norm-version-actions">' + renderVersionActions(n, v, sid, vid) + '</div></div>';
  });
  return h + '</div>';
}
function renderNorms() {
  ...
  grid.querySelectorAll('.norm-info').forEach(function (b) { b.onclick = function (e) { e.preventDefault(); e.stopPropagation(); showNormInfo(b.dataset.id); }; });
}
