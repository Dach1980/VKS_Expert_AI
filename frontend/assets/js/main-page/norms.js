...
function renderNormVersions(norm) {
  ...
  versions.slice().sort(function(a,b){return String(b.effective_from||'').localeCompare(String(a.effective_from||''));}).forEach(function(v){
    ...
    html+='</div></div>';
    html+='<button class="btn btn-secondary btn-sm" onclick="showNormVersionInfo('+JSON.stringify(norm.id)+','+JSON.stringify(v.version_id)+','+JSON.stringify(v.document_id||norm.id)+')">Информация</button></div>';
  });
  return html+'</div>';
}
