...
function renderNorms() {
  ...
  var html=''; norms.forEach(function(norm){var status=norm.status||'pending';var progress=norm.progress==null?normProgress(norm):norm.progress;var p=norm.processing||{};var id=escapeHtmlSafe(JSON.stringify(norm.id));
    ...
    html+='<div class="norm-card-actions">'; if(status!=='indexing'&&status!=='indexed')html+='<button class="btn btn-primary btn-sm" onclick="indexNorm('+id+')">Индексировать</button>'; html+='<button class="btn btn-secondary btn-sm" onclick="showNormInfo('+id+')">Информация</button><button class="btn btn-secondary btn-sm" onclick="deleteNorm('+id+')">Удалить</button></div></div>'; }); grid.innerHTML=html;
}
