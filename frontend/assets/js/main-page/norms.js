...
function getNormByIdLocal(id) {
  return getNormsData().find(function (n) {
...
function changeNumberFromFilename(filename) {
  ...
  var match = stem.match(
    /(?:\bизм(?:енение|енения)?\.?|\bизменени[ея]|\bamendment)\s*№?\s*\.?\s*(\d+)\b/i,
  );
  return match ? match[1] : null;
}
function versionFilename(version) {
  return String(
    (version && (version.original_filename || version.filename)) || '',
  ).trim();
}
...
function isUserCurrent(version) {
  return !!(
    version && String(version.status || '').toLowerCase() === 'current'
  );
}
...
function generationState(version) {
  return String(
    (version &&
      version.processing &&
      version.processing.normative_generation_status) ||
      '',
  ).toLowerCase();
}
function isGenerating(version) {
  return generationState(version) === 'running';
}
function isGenerationValidated(version) {
  return (
    generationState(version) === 'validated' &&
    version.processing &&
    version.processing.normative_generation_valid === true
  );
}
function findVersion(norm, sourceId, versionId) {
  ...
  return (
    versions.find(function (v) {
      return (
        String(v.version_id || v.id) === String(versionId) &&
        String(v.document_id || sourceId) === String(sourceId)
      );
    }) ||
    versions.find(function (v) {
      return String(v.version_id || v.id) === String(versionId);
    })
  );
}
...
function renderGenerationAction(norm, version) {
  ...
  if (isGenerating(version))
    return '<span class="status-badge info">Создание JSON 2.0…</span>';
  if (isGenerationValidated(version))
    return '<span class="status-badge success">JSON 2.0 проверен</span>';
  return (
    '<button type="button" class="btn btn-secondary btn-sm norm-version-generate" data-card="' +
    esc(norm.id) +
    '" data-source="' +
    esc(sid) +
    '" data-version="' +
    esc(vid) +
    '">Создать JSON 2.0</button>'
  );
}
function renderIndexAction(norm, version) {
  var sid = version.document_id || norm.id;
  var vid = version.version_id || version.id;
  if (isIndexing(version))
    return '<span class="status-badge info">Происходит индексация</span>';
  if (!isGenerationValidated(version))
    return '<span class="status-badge info">Сначала создать JSON 2.0</span>';
  if (isIndexed(version))
    return '<span class="status-badge success">Индексировано</span>';
  return (
    '<button type="button" class="btn btn-primary btn-sm norm-version-index" data-card="' +
    esc(norm.id) +
    '" data-source="' +
    esc(sid) +
    '" data-version="' +
...
function renderCurrentAction(norm, version) {
  var sid = version.document_id || norm.id;
  var vid = version.version_id || version.id;
  if (isUserCurrent(version))
    return '<span class="status-badge success">Действующая редакция</span>';
  return (
    '<button type="button" class="btn btn-secondary btn-sm norm-version-activate" data-card="' +
    esc(norm.id) +
    '" data-source="' +
    esc(sid) +
    '" data-version="' +
...
function renderVersionActions(norm, version) {
  ...
  var vid = version.version_id || version.id;
  var indexAction =
    isIndexed(version) || isIndexing(version)
      ? ''
      : renderIndexAction(norm, version);
  return (
    renderGenerationAction(norm, version) +
    indexAction +
    renderCurrentAction(norm, version) +
    '<button type="button" class="btn btn-danger btn-sm norm-version-delete" data-card="' +
    esc(norm.id) +
    '" data-source="' +
    esc(sid) +
    '" data-version="' +
...
function renderNormVersions(norm) {
  ...
  var html =
    '<div id="normVersions-' +
    esc(norm.id) +
    '" class="norm-versions-panel" hidden>' +
  ...
  versions.forEach(function (version) {
    ...
    var viewer =
      '<a class="norm-version-view" href="' +
      esc(pdfUrl(sid, vid)) +
      '" target="_blank" rel="noopener" title="Открыть PDF в новой странице" aria-label="Открыть PDF в новой странице">' +
      searchIcon() +
      '</a>';
    html +=
      '<div class="norm-version-row">' +
      '<div class="norm-version-info-block">' +
      '<div class="norm-version-label">' +
      esc(versionLabel(version)) +
      '</div>' +
      '<div class="norm-version-filename">' +
      viewer +
      '<span class="norm-version-file-name" title="' +
      esc(filename) +
      '">' +
      esc(filename) +
      '</span></div>' +
      '<div class="norm-version-file-date"><span class="norm-version-meta-label">Дата загрузки:</span> ' +
      esc(formatDisplayDate(uploadDate)) +
      '</div>' +
      '<div class="norm-version-pages"><span class="norm-version-meta-label">Количество страниц:</span> 📄 ' +
      esc(pages) +
      '</div>' +
      '<div class="norm-version-index-status"><span class="norm-version-meta-label">Статус:</span> ' +
      renderVersionStatus(version) +
      '</div>' +
      '</div><div class="norm-version-actions">' +
      renderVersionActions(norm, version) +
      '</div></div>';
  });
  return html + '</div>';
}
function renderNorms() {
  ...
  getNormsData().forEach(function (norm) {
    ...
    var displayVersion =
      current || indexedVersion || indexingVersion || versions[0] || null;

    var change = displayVersion
      ? versionChangeNumber(displayVersion)
      : norm.current_change_number != null
        ? String(norm.current_change_number)
        : null;

    var changeText = displayVersion
      ? change === null
        ? ' — Без изменений'
        : ' — Изменение №' + esc(change)
      : ' — Действующая редакция не выбрана';

    var pages = displayVersion
      ? (displayVersion.processing || {}).pages_count ||
        displayVersion.pages_count ||
        0
      : 0;
    ...
    html +=
      '<div class="norm-card" data-norm-card="' +
      esc(norm.id) +
      '">' +
      '<div class="norm-card-header"><div style="flex:1;min-width:0">' +
      '<div class="norm-card-title">' +
      esc(norm.number || norm.id) +
      esc(changeText) +
      '</div>' +
      '<div class="norm-card-subtitle">' +
      esc(norm.title || norm.number || '') +
      '</div></div></div>' +
      '<div class="norm-card-meta"><span>📄 ' +
      esc(pages) +
      ' стр.</span><span>📦 ' +
      esc(versions.length) +
      ' верс.</span></div>' +
      '<div class="norm-card-status">' +
      status +
      '</div>' +
      '<div class="norm-card-actions"><button type="button" class="btn btn-secondary btn-sm norm-documents" data-id="' +
      esc(norm.id) +
      '">Загруженные документы</button></div>' +
      renderNormVersions(norm) +
      '</div>';
  });
  grid.innerHTML =
    html ||
    '<div style="text-align:center;padding:48px;color:var(--text-secondary);">Нормативные документы ещё не загружены.</div>';
  grid.querySelectorAll('.norm-documents').forEach(function (button) {
    button.onclick = function (event) {
      event.preventDefault();
      event.stopPropagation();
      toggleNormVersions(button.dataset.id);
    };
  });
  grid.querySelectorAll('.norm-version-generate').forEach(function (button) {
    button.onclick = function (event) {
      event.preventDefault();
      event.stopPropagation();
      generateNormVersion(
  ...
  grid.querySelectorAll('.norm-version-index').forEach(function (button) {
    button.onclick = function (event) {
      event.preventDefault();
      event.stopPropagation();
      indexNormVersion(
  ...
  grid.querySelectorAll('.norm-version-activate').forEach(function (button) {
    button.onclick = function (event) {
      event.preventDefault();
      event.stopPropagation();
      activateNormVersion(
  ...
  grid.querySelectorAll('.norm-version-delete').forEach(function (button) {
    button.onclick = function (event) {
      event.preventDefault();
      event.stopPropagation();
      deleteNormVersion(
...
async function loadNorms(expandIds) {
  try {
    ...
    setNormsData(
      (data.documents || []).map(function (item) {
        ...
        return {
          ...
          current_change_number: item.current_change_number,
          current_change_date: item.current_change_date,
          status: processing.error
            ? 'error'
            : processing.indexing
              ? 'indexing'
              : processing.vector_index && processing.vector_metadata
                ? 'indexed'
                : 'pending',
    ...
    toast('Не удалось получить нормативную базу: ' + error.message, 'error');
    return [];
  }
}
async function generateNormVersion(cardId, sourceId, versionId) {
  ...
  if (isGenerating(version)) return;
  try {
    ...
    version.processing.normative_generation_status = 'running';
    version.processing.normative_generation_valid = false;
    renderNorms();
    toast(
      'Создание Normative JSON 2.0 запущено: ' +
        (versionFilename(version) || versionId),
      'info',
    );
    pollNormGeneration(cardId, sourceId, versionId);
  } catch (error) {
    toast('Ошибка запуска JSON 2.0: ' + error.message, 'error');
  }
}
function pollNormGeneration(cardId, sourceId, versionId) {
  ...
  normsPollTimers[key] = setInterval(async function () {
    try {
      ...
      if (!version) return;
      version.processing = version.processing || {};
      version.processing.normative_generation_status = data.status || 'unknown';
      version.processing.normative_generation_valid = data.valid === true;
      if (data.status === 'validated' && data.valid === true) {
        clearInterval(normsPollTimers[key]);
        delete normsPollTimers[key];
        toast('Normative JSON 2.0 успешно создан и прошёл проверку', 'success');
      } else if (data.status === 'invalid' || data.status === 'failed') {
        clearInterval(normsPollTimers[key]);
        delete normsPollTimers[key];
        toast(
          'Создание JSON 2.0 завершилось с ошибкой. Проверьте детали.',
          'error',
        );
      }
      renderNorms();
...
async function indexNormVersion(cardId, sourceId, versionId) {
  ...
  try {
    ...
    renderNorms();
    toast(
      'Происходит индексация: ' + (versionFilename(version) || versionId),
      'info',
    );
...
async function activateNormVersion(cardId, sourceId, versionId) {
  ...
  try {
    ...
    if (!response.ok) throw new Error(data.detail || 'HTTP ' + response.status);
    toast('Версия назначена действующей', 'success');
    await loadNorms([cardId]);
  } catch (error) {
    toast('Ошибка смены действующей версии: ' + error.message, 'error');
  }
}
function indexNorm(id) {
  var norm = getNormByIdLocal(id);
  if (!norm || !norm.version_id)
    return toast('Сначала назначьте действующую версию', 'error');
  indexNormVersion(norm.id, norm.id, norm.version_id);
}
function pollNormStatus(cardId, sourceId, versionId) {
  ...
  normsPollTimers[key] = setInterval(async function () {
    try {
      ...
      if (version.processing.error) {
        clearInterval(normsPollTimers[key]);
        delete normsPollTimers[key];
        toast(
          'Индексация завершилась с ошибкой: ' + version.processing.error,
          'error',
        );
      } else if (isIndexed(version)) {
        clearInterval(normsPollTimers[key]);
        delete normsPollTimers[key];
        toast('Индексация выбранной версии завершена', 'success');
      }
      renderNorms();
      var panel = document.getElementById('normVersions-' + cardId);
      if (panel) panel.removeAttribute('hidden');
    } catch (error) {
      console.warn('[Norms] status', error);
    }
  }, 1500);
}
function indexAllNorms() {
  getNormsData().forEach(function (norm) {
    ...
    if (current && !isIndexing(current) && !isIndexed(current))
...
async function deleteNormVersion(cardId, sourceId, versionId) {
  ...
  try {
    var response = await fetch(
      NORMS_API_BASE +
        '/' +
        encodeURIComponent(sourceId || cardId) +
        '?version_id=' +
        encodeURIComponent(versionId),
      { method: 'DELETE' },
    );
    ...
    if (!response.ok) throw new Error(data.detail || 'HTTP ' + response.status);
    toast('Файл удалён: ' + filename, 'success');
    await loadNorms();
  } catch (error) {
...
function deleteNorm(id) {
  var norm = getNormByIdLocal(id);
  if (norm && norm.versions && norm.versions.length)
    deleteNormVersion(
      id,
      norm.id,
      norm.versions[0].version_id || norm.versions[0].id,
    );
}
function handleNormDropzoneClick() {
  ...
  input.onchange = function (event) {
    handleNormFiles(event.target.files);
  };
  input.click();
}
...
function handleNormDrop(event) {
  ...
  if (event.dataTransfer && event.dataTransfer.files.length)
    handleNormFiles(event.dataTransfer.files);
}
...
async function uploadNormFile(file) {
  ...
  var response = await fetch(NORMS_API_BASE + '/upload', {
    method: 'POST',
    body: form,
  });
  var data = {};
  ...
  if (response.status === 409) {
    toast(data.detail || 'Файл «' + file.name + '» уже загружен.', 'warning');
  ...
  toast(
    'Файл «' +
      file.name +
      '» загружен. Версия ожидает выбора действующей редакции.',
    'success',
  );
  return data;
}
...
window.generateNormVersion = generateNormVersion;
...
console.log('[VKS Expert AI][Norms] norms.js v11 loaded');
