...
RECOVERY_PROMPT = """Выполни короткий визуальный проход страницы проектной документации.
...
def run_traced_resilient_check(document_id: str, normative_number: str, progress_callback: Callable[[dict[str, Any]], None] | None = None, skill_id: str = "vk_wastewater") -> dict[str, Any]:
    ...
    def drop(reason: str) -> None: trace["drop_reasons"][reason] = int(trace["drop_reasons"].get(reason, 0)) + 1

    original_strict = impl._strict_candidates; original_filter = impl._filter_skill_candidates; original_bbox = impl._bbox_has_real_evidence; original_retrieve = impl.retrieve_audit_context; original_decide = impl.decide_audit; original_vision = impl._vision_request; original_json_array = impl._json_array

    def needs_recovery(parsed: list[dict[str, Any]]) -> bool:
    ...
    def traced_vision(client, prompt: str, image_path: Path, max_tokens: int = 1200) -> str:
        text = original_vision(client, prompt, image_path, max_tokens); parsed = original_json_array(text)
        if not needs_recovery(parsed): return text
        if not parsed: trace["raw_empty_pages"] += 1
        else: trace["raw_invalid_skill_pages"] += 1
        if len(trace["raw_samples"]) < 5: trace["raw_samples"].append({"kind": "empty_or_invalid_skill", "response_preview": str(text or "")[:1200]})
        recovery = original_vision(client, RECOVERY_PROMPT, image_path, max(2200, max_tokens)); recovered = original_json_array(recovery)
        if recovered:
            trace["visual_recovery_pages"] += 1; trace["visual_recovery_candidates"] += len(recovered)
    ...
    def traced_strict(raw: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for item in raw: bucket(item)["raw_visual_candidates"] += 1
        accepted = []
        for item in raw:
            ...
            if not evidence_text or not title or confidence < 0.35: drop("strict_candidate_validation"); continue
            if not isinstance(bbox, (list, tuple)) or len(bbox) != 4: drop("strict_candidate_bbox_missing"); continue
            if not description or len(description) < 10: drop("strict_candidate_description"); continue
            accepted.append({**item, "evidence_text": evidence_text}); bucket(item)["strict_candidates"] += 1
        trace["strict_candidates"] += len(accepted); return accepted

    def traced_filter(candidates, skill, page_number):
        allowed = {str(x["id"]) for x in skill["checks"]}; accepted = original_filter(candidates, skill, page_number); trace["skill_candidates"] += len(accepted)
    ...
    def traced_retrieve(retrievers, candidate, top_k=6, skill_id="vk_wastewater"):
        trace["rag_calls"] += 1; b = bucket(candidate); b["rag_calls"] += 1; results = original_retrieve(retrievers, candidate, top_k=top_k, skill_id=skill_id)
    ...
    def traced_decide(*args, **kwargs):
        ...
        candidate = args[1] if len(args) > 1 and isinstance(args[1], dict) else {}; b = bucket(candidate); b["decisions"] += 1; b[kind if kind in {"violations", "compliant", "unchecked"} else "unchecked"] += 1; return decision
    ...
    root = Path(__file__).resolve().parents[2] / "knowledge" / "project_documents" / document_id; checkpoint_path = root / "checking" / "first_pass" / "checkpoint.json"
