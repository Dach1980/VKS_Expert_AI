    ...
    try:
        ...
        try:
            report = run_traced_resilient_check(
                document_id,
                normative_number="СП 30.13330.2020",
                progress_callback=lambda data: _progress(job_id, data),
                skill_id=skill_id,
            )
        finally:
        ...
        _update(
            ...
            stage="error",
            percent=100,
            message=f"Ошибка проверки: {error}",
            error=str(error),
            finished_at=datetime.now().isoformat(timespec="seconds"),
            estimated_remaining_seconds=0,
        )
...
def start_check_job(document_id: str, skill_id: str = "vk_wastewater") -> dict[str, Any]:
    ...
    thread = threading.Thread(
        target=_worker,
        args=(job_id, document_id, skill_id),
        daemon=True,
        name=f"check-{job_id[:8]}",
    )
    thread.start()
