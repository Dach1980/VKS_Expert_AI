...
def run_first_pass_api(document_id: str, normative_number: str = DEFAULT_NORM_NUMBER) -> dict[str, Any]:
    ...
    report = {
        ...
        "results": findings,
        "summary": {
            "pages": len(pages), "total": len(findings),
            "violations": len(violations),
            "critical": sum(x["severity"] == "critical" for x in violations),
            "major": sum(x["severity"] == "major" for x in violations),
            "minor": sum(x["severity"] == "minor" for x in violations),
            "compliant": sum(x["type"] == "compliant" for x in findings),
            "unchecked": sum(x["type"] == "unchecked" for x in findings),
        },
    }
