...
def _load_checkpoint(path: Path, document_id: str, pdf_name: str) -> dict[str, Any]:
    empty = {"status": "running", "document_id": document_id, "document_name": pdf_name,
             "pages_completed": 0, "completed_pages": [], "findings": [], "last_error": None}
    if not path.exists():
        return empty
    try:
...
def _indexed_norms(storage: KnowledgeStorage) -> list[tuple[dict[str, Any], dict[str, Any], Retriever]]:
    ...
    for document in storage.registry.get_all_documents():
        try:
            ...
            if not (paths.embeddings / "index.faiss").exists() or not (paths.embeddings / "metadata.json").exists():
                continue
...
def _multi_norm_search(retrievers, query: str, top_k: int = MAX_NORM_RESULTS) -> list[dict[str, Any]]:
    ...
    for document, version, retriever in retrievers:
        ...
        try:
            for item in retriever.search(query, top_k=top_k):
                ...
                item["version"] = str(version.get("id") or item.get("version") or "")
                merged.append(item)
        except Exception:
            continue
...
def _multi_context(results: list[dict[str, Any]]) -> str:
    ...
    for item in results:
        ...
        if text:
            parts.append(f"{item.get('norm_number', 'СП')}, версия {item.get('version', '—')}, стр. {item.get('page', '—')}: {text}")
    value = "\n\n".join(parts)
...
def _bbox_has_real_evidence(image_path: Path, bbox: list[float]) -> bool:
    try:
        with Image.open(image_path).convert("L") as image:
            ...
            if not pixels:
                return False
            return sum(1 for value in pixels if value < 245) / len(pixels) >= 0.001
    except Exception:
        return False


def _build_report(document_id: str, pdf_name: str, norms: list[tuple[dict, dict, Retriever]], total_pages: int,
                  findings: list[dict[str, Any]], failed_pages: list[int]) -> dict[str, Any]:
    ...
    return {
        ...
        "checked_at": datetime.now().isoformat(timespec="seconds"), "status": "completed",
        ...
        "normative_basis": basis, "results": findings,
        ...
        "summary": {"pages": total_pages, "total": len(findings), "violations": len(violations),
                    "critical": sum(x.get("severity") == "critical" for x in violations),
                    "major": sum(x.get("severity") == "major" for x in violations), "minor": sum(x.get("severity") == "minor" for x in violations),
                    "compliant": sum(x.get("type") == "compliant" for x in findings), "unchecked": sum(x.get("type") == "unchecked" for x in findings)},
    }


def run_resilient_check(document_id: str, normative_number: str = DEFAULT_NORM_NUMBER,
    ...
    progress(stage="preparing", percent=1, current_page=0, total_pages=0, message="Подготовка проверки и нормативной базы…")
    ...
    norm_names = ", ".join(str(d.get("number")) for d, _, _ in norms)
    progress(stage="visual", percent=2, current_page=max(completed_pages, default=0), total_pages=total_pages,
             message=(f"Возобновляю проверку: завершено {len(completed_pages)} из {total_pages}. Нормативы: {norm_names}." if completed_pages else f"Подготовлено страниц: {total_pages}. Нормативы: {norm_names}. Начинаю инженерный аудит…"))
    ...
    for page_index, page in enumerate(pages, start=1):
        ...
        for attempt in range(1, MAX_PAGE_RETRIES + 1):
            ...
            try:
                progress(stage="visual", percent=page_start, current_page=page_index, total_pages=total_pages, retry=attempt if attempt > 1 else 0,
                         message=(f"Повторная обработка страницы {page_index} из {total_pages} (попытка {attempt}/{MAX_PAGE_RETRIES})…" if attempt > 1 else f"Инженерный аудит страницы {page_index} из {total_pages}…"))
                candidates = _strict_candidates(_json_array(_vision_request(client, build_vk_audit_prompt(page.page), Path(page.image_path), 1400)))[:MAX_PAGE_CANDIDATES]
                progress(stage="normative", percent=min(98, page_start + 1), current_page=page_index, total_pages=total_pages,
                         message=f"Страница {page_index}: найдено инженерных фактов {len(candidates)}. Сверяю с {len(norms)} нормативными документами…")
                ...
                for candidate in candidates:
                    ...
                    norm_results = _multi_norm_search(norms, query, top_k=MAX_NORM_RESULTS)
                    norm_text = _multi_context(norm_results)
                    if not norm_text:
                        continue
                    decision = _decision(client, candidate, norm_text)
                    if decision.get("type") not in {"violation", "compliant", "unchecked"}:
                        decision["type"] = "unchecked"
                    if decision.get("type") == "violation" and float(decision.get("confidence") or 0) < 0.55:
                        decision["type"] = "unchecked"
                    ...
                    page_findings.append({
                        "id": finding_id, "type": decision.get("type", "unchecked"), "docId": document_id, "docName": pdf_path.name,
                        ...
                        "norm": str(decision.get("norm") or (norm_results[0].get("norm_number") if norm_results else normative_number)),
                        "clause": str(decision.get("clause") or ""), "severity": str(decision.get("severity") or "minor"), "page": page.page,
                        "bbox": bbox, "evidence_image": evidence_image,
                        "image": f"{REPORT_API_BASE}/api/reports/evidence/{document_id}/{evidence_path.name}",
                        "evidence_text": str(candidate.get("evidence_text") or ""), "confidence": decision.get("confidence", candidate.get("confidence")),
                ...
                page_success = True
                progress(stage="visual", percent=min(98, 2 + int(page_index / max(total_pages, 1) * 96)), current_page=page_index, total_pages=total_pages,
                         page_completed=True, message=f"Страница {page_index} из {total_pages} завершена. Значимых результатов: {len(page_findings)}.")
                break
    ...
    report = _build_report(document_id, pdf_path.name, norms, total_pages, findings, failed_pages)
