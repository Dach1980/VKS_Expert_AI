...
from app.checking.first_pass import CHECK_DPI, DEFAULT_NORM_NUMBER, MAX_NORM_RESULTS, REPORT_API_BASE, _decision, _json_array, _strict_candidates, _vision_request
...
def _load_checkpoint(path: Path, document_id: str, pdf_name: str) -> dict[str, Any]:
    ...
    try:
        ...
        value.setdefault("completed_pages", []); value.setdefault("findings", []); value.setdefault("pages_completed", len(value["completed_pages"]))
        return value
...
def _bbox_has_real_evidence(image_path: Path, bbox: list[float]) -> bool:
    try:
        with Image.open(image_path).convert("L") as image:
            ...
            crop=image.crop((int(x1),int(y1),int(x2),int(y2))); pixels=list(crop.getdata())
...
def run_resilient_check(document_id: str, normative_number: str=DEFAULT_NORM_NUMBER, progress_callback: ProgressCallback|None=None) -> dict[str,Any]:
    ...
    evidence_dir=root/"checking"/"first_pass"; evidence_dir.mkdir(parents=True,exist_ok=True); checkpoint_file=_checkpoint_path(evidence_dir)
    storage=KnowledgeStorage(); norms=_indexed_norms(storage)
    ...
    pages=render_pdf_pages(pdf_path,evidence_dir,dpi=CHECK_DPI); total_pages=len(pages); checkpoint=_load_checkpoint(checkpoint_file,document_id,pdf_path.name)
    completed_pages={int(x) for x in checkpoint.get("completed_pages",[])}; findings=[x for x in checkpoint.get("findings",[]) if isinstance(x,dict)]; next_finding_id=max((int(x.get("id",0) or 0) for x in findings),default=0)+1
    norm_names=", ".join(str(d.get("number")) for d,_,_ in norms)
    progress(stage="visual",percent=2,current_page=max(completed_pages,default=0),total_pages=total_pages,message=(f"Возобновляю проверку: завершено {len(completed_pages)} из {total_pages}. Нормативы: {norm_names}." if completed_pages else f"Подготовлено страниц: {total_pages}. Нормативы: {norm_names}. Начинаю инженерный аудит…"))
    failed_pages=[]
    for page_index,page in enumerate(pages,start=1):
        ...
        for attempt in range(1,MAX_PAGE_RETRIES+1):
            ...
            try:
                progress(stage="visual",percent=page_start,current_page=page_index,total_pages=total_pages,retry=attempt if attempt>1 else 0,message=(f"Повторная обработка страницы {page_index} из {total_pages} (попытка {attempt}/{MAX_PAGE_RETRIES})…" if attempt>1 else f"Инженерный аудит страницы {page_index} из {total_pages}…"))
                ...
                progress(stage="normative",percent=min(98,page_start+1),current_page=page_index,total_pages=total_pages,message=f"Страница {page_index}: найдено инженерных фактов {len(candidates)}. Сверяю с {len(norms)} нормативными документами…")
                page_findings=[]
                for candidate in candidates:
                    ...
                    norm_results=retrieve_audit_context(norms,candidate,top_k=MAX_NORM_RESULTS)
                ...
                findings.extend(page_findings); next_finding_id+=len(page_findings); completed_pages.add(page_index); checkpoint.update({"status":"running","pages_completed":len(completed_pages),"completed_pages":sorted(completed_pages),"findings":findings,"last_error":None}); _write_json(checkpoint_file,checkpoint); page_success=True
                progress(stage="visual",percent=min(98,2+int(page_index/max(total_pages,1)*96)),current_page=page_index,total_pages=total_pages,page_completed=True,message=f"Страница {page_index} из {total_pages} завершена. Значимых результатов: {len(page_findings)}.")
                break
