...
def _load_checkpoint(path: Path, document_id: str, pdf_name: str, skill_id: str) -> dict[str, Any]:
    empty = {
        "status": "running", "document_id": document_id, "document_name": pdf_name,
        "skill_id": skill_id, "pages_completed": 0, "completed_pages": [],
        "findings": [], "last_error": None,
    }
    if not path.exists():
        return empty
    try:
...
def _finalise_decision(decision: dict[str, Any], candidate: dict[str, Any], requirements: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if decision["type"] in {"violation", "compliant"}:
        if not valid:
            ...
            # The LLM is not allowed to invent the citation: take it from RAG.
            selected = valid[0]
...
def _candidate_key(candidate: dict[str, Any]) -> tuple[str, str, str]:
    ...
    parameter = " ".join(str(candidate.get(x) or "").lower() for x in ("parameter", "project_value", "source_context", "evidence_text"))
    # Keep separate actual project facts while collapsing repeated OCR descriptions.
    import re
    parameter = re.sub(r"\s+", " ", parameter).strip()
    return check_id, parameter[:700], str(candidate.get("page") or "")
...
def _dedupe_findings(findings: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for finding in findings:
        key = (
            ...
            str(finding.get("project_value_raw") or finding.get("project_value") or "").strip().lower(),
        )
...
def run_resilient_check(document_id: str, normative_number: str = DEFAULT_NORM_NUMBER, progress_callback: ProgressCallback | None = None, skill_id: str = "vk_wastewater") -> dict[str, Any]:
    ...
    progress(stage="preparing", percent=1, current_page=0, total_pages=0, message=f"Подготовка Skill «{skill['name']}» и {len(skill['checks'])} инженерных проверок…")
    ...
    norm_names = ", ".join(skill["normative_documents"] if isinstance(skill.get("normative_documents"), str) else skill.get("normative_documents", {}).keys())
    progress(stage="visual", percent=2, current_page=max(completed_pages, default=0), total_pages=total_pages, message=f"Подготовлено страниц: {total_pages}. Skill содержит {len(skill['checks'])} предметных проверок. Начинаю аудит…")
    ...
    for page_index, page in enumerate(pages, start=1):
        ...
        for attempt in range(1, MAX_PAGE_RETRIES + 1):
            ...
            try:
                progress(stage="visual", percent=page_start, current_page=page_index, total_pages=total_pages, retry=attempt if attempt > 1 else 0, message=f"Аудит страницы {page_index} из {total_pages}: один визуальный проход по {len(skill['checks'])} проверкам…")
                raw = _json_array(_vision_request(client, build_vk_audit_prompt(page.page, skill_id), Path(page.image_path), 1400))
                candidates = _filter_skill_candidates(_strict_candidates(raw), skill, page.page)
                progress(stage="normative", percent=min(98, page_start + 1), current_page=page_index, total_pages=total_pages, message=f"Страница {page_index}: {len(candidates)} предметных кандидатов Skill. Выполняю RAG и decision…")
                page_findings = []
                for candidate in candidates:
                    ...
                    page_findings.append({
                        ...
                        "check_id": str(candidate.get("check_id") or ""),
                        "check_name": str(candidate.get("check_name") or ""),
                        "docId": document_id,
                        ...
                        "sheet": str(decision.get("sheet") or page.page),
                        "norm": table_row.norm,
                        "clause": table_row.clause,
                        "parameter": table_row.parameter,
                        "project_value": table_row.project_value_raw,
                        "project_value_raw": table_row.project_value_raw,
                        "project_unit": table_row.project_unit,
                        "normative_requirement": table_row.normative_requirement,
                        "normative_value": table_row.normative_value_raw,
                        "normative_value_raw": table_row.normative_value_raw,
                        "normative_unit": table_row.normative_unit,
                        "comparison": table_row.comparison,
                        "project_value_normalized": table_row.project_value,
                        "project_kind": table_row.project_kind,
                        "normative_value_normalized": table_row.normative_value,
                        "normative_kind": table_row.normative_kind,
                        "source_row": table_row.source_row,
                        "source_context": table_row.source_context,
                        "table_check": table_row.to_dict(),
                        ...
                        "evidence_text": table_row.evidence_text,
                        "confidence": table_row.confidence,
                        "normative_route": candidate.get("normative_route"),
    ...
    _write_json(checkpoint_file, checkpoint)
    progress(stage="completed", percent=100, current_page=total_pages, total_pages=total_pages, page_completed=True, message=f"Проверка завершена. Обработано {total_pages} страниц, Skill-проверок: {len(skill['checks'])}. Отчёт готов.")
    return report
