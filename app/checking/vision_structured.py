...
def structured_vision_request(client: LMStudioClient, prompt: str, image_path: Path, max_tokens: int = 1400) -> str:
    ...
    payload: dict[str, Any] = {
        ...
        "messages": [
            ...
            {"role": "user", "content": [{"type": "text", "text": prompt}, {"type": "image_url", "image_url": {"url": image_data}}]},
        ],
        "temperature": 0.1,
    ...
    if client.cancel_event is not None:
        payload["stream"] = True
        return client._chat_stream(f"{client.base_url}/chat/completions", payload)
    ...
    if not response.ok:
        ...
        if len(body) > 1000:
            body = body[:1000] + "..."
        print(f"[NORMCONTROL] VISION_HTTP_ERROR status={response.status_code} model={client.model} body={body}", flush=True)
    response.raise_for_status()
    data = response.json()
    actual_model = data.get("model")
    if actual_model and actual_model != client.model:
        raise RuntimeError(f"LM Studio вернул другую модель: запрошена «{client.model}», фактически «{actual_model}»")
    ...
    if content:
        return content
    print(f"[NORMCONTROL] VISION_STRUCTURED_EMPTY model={client.model} reasoning_present={int(bool(str(message.get('reasoning_content') or '').strip()))}", flush=True)
    return ""
