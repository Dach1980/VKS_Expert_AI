...
def _report_for_document(document_id:str)->dict:
    ...
    report["template"]=REPORT_TEMPLATE; return report
@router.get("")
def list_reports():
    items=[]
    if DOCUMENTS_ROOT.exists():
        for root in DOCUMENTS_ROOT.iterdir():
            if root.is_dir():
                ...
                if report: report["template"]=REPORT_TEMPLATE; items.append(report)
    items.sort(key=lambda x:str(x.get("checked_at") or ""),reverse=True)
    return {"reports":items}
@router.get("/{document_id}")
def get_report(document_id:str):return {"success":True,**_report_for_document(document_id)}
@router.post("/create/{document_id}")
def create_report(document_id:str):return {"success":True,**_report_for_document(document_id)}
@router.post("/pdf")
def export_pdf(report:dict):
    try:data=build_pdf(report)
    except Exception as error:raise HTTPException(status_code=500,detail=f"Не удалось сформировать PDF: {error}") from error
    document_id=str(report.get("document_id","report")); return StreamingResponse(io.BytesIO(data),media_type="application/pdf",headers={"Content-Disposition":f'attachment; filename="project-expert-ai-check-{document_id}.pdf"'})
@router.post("/docx")
def export_docx(report:dict):
    try:data=build_docx(report)
    except Exception as error:raise HTTPException(status_code=500,detail=f"Не удалось сформировать Word: {error}") from error
    document_id=str(report.get("document_id","report")); return StreamingResponse(io.BytesIO(data),media_type="application/vnd.openxmlformats-officedocument.wordprocessingml.document",headers={"Content-Disposition":f'attachment; filename="project-expert-ai-check-{document_id}.docx"'})
