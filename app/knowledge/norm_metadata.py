...
def _extract_from_text(text: str) -> dict[str, str]:
    ...
    result: dict[str, str] = {}
    number = re.search(r"\b((?:СП|ГОСТ\s*Р?|СНиП|ТР|ФЗ)\s*\d+(?:\.\d+)+)\b", text, re.IGNORECASE)
    if number:
...
def _extract_from_json(path: Path) -> dict[str, str]:
    ...
    result: dict[str, str] = {}
    strings: list[str] = []
    preferred = {"document_number":"number","norm_number":"number","normative_number":"number","standard_number":"number","document_title":"title","norm_title":"title","change_number":"change_number","amendment_number":"change_number","revision_number":"change_number","change_date":"change_date","amendment_date":"change_date","revision_date":"change_date"}
    for key, value in _walk_strings(data):
        if not isinstance(value, str) or not value.strip():
            continue
        value = value.strip();strings.append(value);target=preferred.get(key.lower().strip())
        if target == "number" and re.search(r"(?:СП|ГОСТ|СНиП|ТР|ФЗ)\s*\d+(?:\.\d+)+", value, re.IGNORECASE):result.setdefault("number", re.sub(r"\s+", " ", value))
...
def extract_version_metadata(pdf_path: Path | str, parsed_path: Path | str | None = None) -> dict[str, Any]:
    ...
    pdf_path=Path(pdf_path);parsed_path=Path(parsed_path) if parsed_path else None;result=_extract_from_json(parsed_path) if parsed_path else {};page_count=0
    if pdf_path.exists():
        try:
            import pymupdf
            with pymupdf.open(pdf_path) as document:
                page_count=len(document);sample_text="\n".join(document[index].get_text("text") for index in range(min(len(document),5)))
